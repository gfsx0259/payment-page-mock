...
export default class BaseStore {
  ...
  actions() {
    return {
      fetch: async ({ state, commit }) => {
        ...
        const url = state.relationId
          ? `${this.endpoint()}/${state.relationId}`
          : this.endpoint();
      ...
      remove: async ({ commit, dispatch }, id) => {
        ...
        HttpClient.delete(`${this.endpoint()}/${id}`).catch(({ response }) => {
          ...
          commit(
            "setMessage",
            { text: `Request failed: ${data.error.message}` },
            { root: true }
          );
          dispatch("fetch");
        });
      ...
      validate: ({ state, commit }) => {
        ...
        Object.entries(this.rules()).map((config) => {
      ...
      request: async ({ commit, dispatch }, config) => {
        try {
          ...
          commit(
            "setMessage",
            { text: this.buildSuccessMessage(config.method) },
            { root: true }
          );
        } catch (error) {
  ...
  buildSuccessMessage(httpMethod) {
    return `${_.upperFirst(this.endpoint())} ${
      CRUD_METHOD_LABEL[httpMethod]
    } successfully`;
  }
